...
cv2.putText(paintWindow, "ERASE", (520, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
...
while ret:
    ...
    cv2.putText(frame, "ERASE", (520, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
    ...
    if result.multi_hand_landmarks:
        ...
        red_index += 1
        epoints.append(deque(maxlen=1024))
        erase_index += 1

    points = [bpoints, gpoints, rpoints, epoints]
...
hand_accuracy = (correct_detections / total_frames) * 100
gesture_accuracy = (correct_gestures / gesture_attempts) * 100 if gesture_attempts > 0 else 0

print(f"Hand Detection Accuracy: {hand_accuracy:.2f}%")
print(f"Gesture Recognition Accuracy: {gesture_accuracy:.2f}%")
